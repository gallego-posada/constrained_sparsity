...
declare -a model_types=("MLP" "LeNet")
...
for model_type in ${model_types[@]}; do
    ...
    model_arg="--model_type $model_type"
    ...
    for seed in ${seeds[@]}; do
        ...
        python_args="${yaml_arg} ${seed_arg} ${checkpoint_arg} ${wandb_arg} ${model_arg}"

        for reg_type in ${reg_types[@]}; do

            for target_density in ${target_densities[@]}; do

                # Set up _tdst_list (3 layers for MLPs, 4 layers for LeNet)
                _tdst_list=$( mnist_regularization_list "${reg_type}" "${target_density}" "${model_type}" )
            ...
            for lmbda in ${lmbdas[@]}; do
                ...
                # Set up _lmbda_list (3 layers for MLPs, 4 layers for LeNet)
                _lmbda_list=$( mnist_regularization_list "${reg_type}" "${lmbda}" "${model_type}" )
